                 ...
                 static_cast<int>(std::max(GLInterface->GetBackBufferHeight(), 1u)))
{
  ...
  {
    g_Config.backendInfo.bSupportsFramebufferFetch = true;
    g_Config.backendInfo.bSupportsInOutFramebufferFetch = true;
  }
  else if (GLExtensions::Supports("GL_EXT_shader_framebuffer_fetch"))
  {
    g_Config.backendInfo.bSupportsFramebufferFetch = true;
    g_Config.backendInfo.bSupportsInOutFramebufferFetch = false;
  }
  else
  {
